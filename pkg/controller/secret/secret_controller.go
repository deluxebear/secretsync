...
func createSecret(secret *corev1.Secret, name string, namespace string) (*corev1.Secret, error){
	labels := map[string]string{
		"secretsync.jetems.com/replicated-from": fmt.Sprintf("%s.%s", secret.Namespace, secret.Name),
	}
	annotations := map[string]string{
		"secretsync.jetems.com/replicated-time": time.Now().Format("Mon Jan 2 15:04:05 MST 2006"),
		"secretsync.jetems.com/replicated-resource-version": secret.ResourceVersion,
	}
...
func (r *ReconcileSecret) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if rep, ok := instance.Annotations["secretsync.jetems.com/replicate"]; ok {
		if rep == "true" {
			if tgts, ok := instance.Annotations["secretsync.jetems.com/replicate-to"]; ok {
				namespaceType := &corev1.NamespaceList{}
				var err = r.client.List(context.TODO(),  namespaceType, &client.ListOptions{})
				if err != nil {
					return reconcile.Result{}, err
				}
